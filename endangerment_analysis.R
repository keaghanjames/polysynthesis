setwd("") #you'll need to set this yourself
library(ordinalNet)
library(ivreg)
library(nloptr)
...
#if you want to add interaction terms for each of the global regions use the below code
...
y <- data$EGIDS_tr
levels(y)[1:6] <- "6a" #and we also collapse the first six levels into a single level to be consistent with Bromham et al. 2022. 

#we extract our predictors
X <- data[,c(model,colnames(data)[2808])] #model is the best model for language endangerment from Bromham et al. 2022.

#if you're using the regional interaction terms use the below instead
...
#now we can run the lasso regularization
...
saveRDS(best.poly.model, 'best.polybord.model.rds')

#if you want to save the version with interactions
...
#if you want to do the same with the model with regional interactions you need to increase the 
#degrees of freedom to 13
# test_stat <- 2 * (best.poly.model$fit$loglik[20]
#                   - best.model$fit$loglik[20])
# p_value <- pchisq(test_stat, 13, lower.tail = FALSE)
# test_stat
# p_value
